...
int detection_L_d = 675;      //625
int detection_rearR_d = 0;    //digital sensor, distance is 0 or 1 if it detects something
int detection_rearL_d = 700;  //650
int detection_diagR_d = 0;    //digital sensor, distance is 0 or 1 if it detects something
int detection_diagL_d = 750;  //700

//                                    //                            //
//                                    // CHANGE THIS VARIABLE BELOW //
//                                    //                            //

int time_CELL = -200;  //Useful to avoid considering the black lines as the BLACK CELL, the variable was found empirically
...
void waitSixtySeconds() {
  ...
  if (millis() > limiteur1) {  //During 10s we follow the function 'goCrazy', and when 10s have passed we switch mode
    switchMode = false;
    limiteur1 = limiteur2 + 60000;
  }
  if (millis() > limiteur2) {  //During 60s we follow the function 'wall_following', and when 60s have passed we switch mode
    switchMode = true;
...
void red_found() {                                       //Turn true FULL_STOP if we're on the RED cell, end of the maze
  ...
  if (vision_color() == 2) {         //If we see RED
    ...
    if (currentTime - millis() < time_CELL) {  //If we see RED for a certain period of time // (currentTime - millis() < -(125000 / (speed / 5)))
...
void noBlackWall() {
  ...
  if (vision_color() == 2) {          //If we see RED
    ...
    if (currentTime - millis() < time_CELL) {  //If we see BLACK for a certain period of time
      ...
      time_CELL = -2400;                       //Increase the time it take to consider the BLACK cell to be the right one because mistakes happen more often
    }
  }
}
